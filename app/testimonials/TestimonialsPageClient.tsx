                  ...
                  <Card className="overflow-hidden cursor-pointer group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 w-full sm:w-auto">
                    <CardContent className="p-0 relative">
                      ...
                      <div className="aspect-[9/16] w-full sm:w-[280px] relative overflow-hidden rounded-lg">
                        {/* Real YouTube thumbnail instead of a generic gradient placeholder —
                            free to load (served from YouTube's CDN, zero cost to our bandwidth)
                            and signals there's an actual video before the visitor even clicks. */}
                        <Image
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center p-6">
                          ...
                          {/* Video info */}
                          <h3 className="font-bold text-white text-lg mb-2 text-center drop-shadow-md">{video.title}</h3>
                          <p className="text-sm text-white/90 mb-3 text-center drop-shadow-md">{video.description}</p>
            ...
            {filteredTestimonials.map((image, index) => (
              <Dialog key={index}>
                <DialogTrigger asChild>
                  ...
                  >
                    <CardContent className={`p-0 relative ${viewMode === "grid" ? "aspect-[9/16]" : ""}`}>
                      <Image
                        ...
                        className={`w-full object-cover group-hover:scale-110 transition-transform duration-300 ${
                          viewMode === "masonry" ? "h-auto" : "absolute inset-0 h-full"
                        }`}
                        loading="lazy"
                        quality={75}
